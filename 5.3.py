...
#    Format (quantity optional; defaults to 1):
...
#   • Returned late (after 2pm): +50% of daily_rate * quantity (once per item line)
...
def parse_csv_line(line):
    """Basic CSV parser supporting quotes and commas. Returns list of fields."""
    fields = []
    buf = []
    in_quotes = False
    i = 0
...
def parse_item_line(raw):
    """
    Accepts:
      code, nights, overdue_days, returned_late
    ...
    if len(parts) not in (4, 5):
        return None, "Expected 4 or 5 fields. E.g. DCH, 3, 0, n  OR  DCH, 3, 2, 0, n"
    code = parts[0].upper()
    item = find_item(code)
    if not item:
...
def run_hire_flow():
    ...
    print("\nEnter item lines (one per line). Format:")
...
def run_earnings_report():
    ...
    for r in HIRE_RECORDS:
        ...
        grand += total_p
        print(f"{r['customer_id']:>3}  {r['customer_name'][:24]:<24}  {items_count:>5}  {money(total_p):>11}")
    print("-" * 54)
    print(f"TOTAL EARNINGS: {money(grand)}")
